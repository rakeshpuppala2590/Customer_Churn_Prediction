...
xgbooost_model = load_model("xgb_model.pkl")
...
voting_classifier_model = load_model('voting_hard_classifier.pkl')

xgbooost_smote_model = load_model('xgboost-smote.pkl')

xgbooost_featureEngineered_model = load_model('xgboost-featureEngineered.pkl')
...
def make_preditions(input_df, input_dict):
    probabiities = {
        'xgboost: ': xgbooost_model.predict_proba(input_df)[0][1],
        'K-Nearest Neighbors: ': knn_model.predict_proba(input_df)[0][1],
        'random_forest: ': random_forest_model.predict_proba(input_df)[0][1]
    }
    avg_probability = np.mean(list(probabiities.values()))
    ...
    with col2:
        fig_probs = ut.create_model_probability_chart(probabiities)
        st.plotly_chart(fig_probs, use_container_width=True)

    # st.markdown("### Model Probabilities")

    # for model, prob in probabiities.items():
...
def explain_prediction(probability, input_dict, surname):
    ...
    Your machine learning model has predicted that a customer named {surname} has a {round(probability * 100, 1)}% probability of churning.

    Here is the customer's information:
    {input_dict}

    Here are the machine learning model's top 10 most important features for predicting churning:
    ...
    Don't mention the probability of churning, the machine learning model, or say anything like "Based on the machine learning model's prediction and top 10 most important features". Just explain the prediction.
    ...
    # Send the prompt to the AI model (client.chat.create call)
...
Make sure to list out a set of incentives to stay based on their information, in bullet point format. Don't ever mention the probability of churning, or the machine learning model to the customer."""
    ...
    # Calling the Llama model to get the email content
    raw_response = client.chat.completions.create(
    ...
    # Printing the email prompt for debugging
    print("\n\nEMAIL PROMPT", prompt)
...
if selected_customer_option:
    ...
    input_df, input_dict = prepare_input(credit_score, location, gender, age,
                                         ...
                                         estimated_salary)
    avg_probability = make_preditions(input_df, input_dict)
